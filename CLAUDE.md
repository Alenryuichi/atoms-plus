...
## 🛠️ CLI 交互问题修复指南 (Agent Development)

### 问题背景

OpenHands Agent 通过 **tmux** 执行 bash 命令 (`openhands/runtime/utils/bash.py`)。tmux 创建伪终端 (PTY)，导致 CLI 工具检测到 `process.stdin.isTTY = true`，即使设置了 `CI=true` 也会进入交互模式。

### 执行链路

```
Agent Action (CmdRunAction)
    ↓
ActionExecutionServer.run()
    ↓
BashSession.execute()  ← libtmux
    ↓
pane.send_keys(command)  ← PTY 环境
    ↓
CLI 工具检测 isTTY = true → 触发交互提示
```

### 已知问题案例

#### 1. create-vite 的 "Use Vite 8 beta?" 提示
...
**根因**：`create-vite` 源码忽略 `CI` 环境变量，直接检测 TTY：
```javascript
// create-vite/dist/index.js
let s = Q.interactive;  // 命令行 --interactive 参数
let l = s ?? process.stdin.isTTY;  // 👈 CI=true 无效
```

**修复**：添加 `--no-interactive` 参数
```bash
# ❌ 会超时
npm create vite@latest app -- --template react-ts
...
| 方法 | 适用场景 | 示例 |
|------|----------|------|
| **显式非交互参数** | CLI 支持 `--no-interactive` / `--yes` / `-y` | `npm create vite ... --no-interactive` |
| **环境变量** | CLI 检查 `CI` / `TERM` | `CI=true npm install` |
| **管道输入** | 需要自动回答 | `echo "y" \| some-cli` |
| **expect/unbuffer** | 复杂交互场景 | 最后手段，尽量避免 |

### 常见 CLI 非交互参数速查

| CLI 工具 | 非交互参数 | 备注 |
|----------|-----------|------|
| `npm` | `--yes` / `-y` | 也可 `npm config set yes true` |
| `npx` | `--yes` | 跳过 "Ok to proceed?" |
| `create-vite` | `--no-interactive` | **不检查 CI 环境变量** |
| `create-react-app` | 无需 (已稳定) | - |
| `vue create` | `--default` / `--preset` | 跳过交互式选择 |
| `ng new` | `--skip-confirmation` | Angular CLI |
| `yarn` | `--non-interactive` | Yarn v1+ |
| `pnpm` | 默认非交互 | - |
| `git` | 各命令不同 | 如 `git commit -m "..."` |

### 修复流程

1. **诊断**：确认是 TTY 检测导致的交互提示
2. **查文档**：搜索 CLI 的 `--no-interactive` 或 `--yes` 参数
3. **查源码**：如文档不全，检查 CLI 源码的 `isTTY` / `interactive` 逻辑
4. **更新 Microagent**：在 `.openhands/microagents/` 中的命令示例添加非交互参数
5. **更新 Skills**：在 `.claude/skills/` 中同步更新

### 相关文件

- `openhands/runtime/utils/bash.py` - BashSession，使用 libtmux
- `.openhands/microagents/scaffolding.md` - Vibe Coding 命令示例
- `atoms_plus/roles/vibe_coding_instructions.py` - 角色指令中的命令
