    ...
    if (isOpenHandsEvent(event)) {
      ...
      if (
        ...
        if (!isCommandAction(event)) {
          ...
          // Invalidate preview workspace files cache
          queryClient.invalidateQueries(
            { queryKey: ["workspace-files", conversationId] },
            { cancelRefetch: false },
          );
          // Invalidate only the specific file content that changed (not all files)
          // Use fileToInvalidate (without /workspace/ prefix) to match queryKey in useWorkspaceFileContent
          queryClient.invalidateQueries(
            {
              queryKey: [
