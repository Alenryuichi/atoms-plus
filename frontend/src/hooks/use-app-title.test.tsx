...
describe("useAppTitle", () => {
  ...
  it("should return 'Atoms Plus' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Atoms Plus"));
  });

  it("should return 'Atoms Plus' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
  ...
  it("should return '{some title} | Atoms Plus' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Atoms Plus"),
    );
  });

  it("should return '{some title} | Atoms Plus' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
