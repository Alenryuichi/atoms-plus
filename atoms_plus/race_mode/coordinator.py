...
class RaceCoordinator:
    ...
    # 支持的模型列表 - LiteLLM 格式 (阿里百炼 Coding API)
    SUPPORTED_MODELS = {
        # ========== 阿里百炼 Coding API (推荐) ==========
        # Priority: MiniMax-M2.5 > glm-5 > qwen3-coder-plus
        'openai/MiniMax-M2.5': {'provider': 'minimax', 'display': 'MiniMax M2.5 ⭐'},
        'openai/glm-5': {'provider': 'zhipu', 'display': '智谱 GLM-5 ⭐'},
        'openai/qwen3-coder-plus': {
            'provider': 'aliyun',
            'display': '通义 Coder Plus ⭐',
        },
        'openai/qwen3.5-plus': {'provider': 'aliyun', 'display': '通义千问 3.5 Plus'},
        'openai/qwen3-max-2026-01-23': {
            'provider': 'aliyun',
            'display': '通义千问 3 Max',
        },
        'openai/qwen3-coder-next': {'provider': 'aliyun', 'display': '通义 Coder Next'},
        'openai/glm-4.7': {'provider': 'zhipu', 'display': '智谱 GLM-4.7'},
        'openai/kimi-k2.5': {'provider': 'kimi', 'display': 'Kimi K2.5'},
        # ========== 国际模型 (需要额外购买) ==========
        # Anthropic
    ...
    # 默认模型组合 - 使用阿里百炼 Coding API 推荐模型
    DEFAULT_MODELS = ['openai/MiniMax-M2.5', 'openai/glm-5', 'openai/qwen3-coder-plus']
    ...
    async def _call_model(
        ...
        try:
            ...
            if model.startswith('openai/qwen'):
                ...
                call_kwargs['api_key'] = os.getenv('DASHSCOPE_API_KEY') or os.getenv(
                    'LLM_API_KEY'
                )
