...
```
1. Request Conversation → POST /api/v1/app-conversations
2. Create Sandbox       → Daytona SDK creates isolated container
                        → Status: WORKING, sandbox_id assigned
3. Sandbox Running      → Agent Server starts on port 8000
                        → Status: RUNNING
4. Execute Tasks        → Agent executes code in isolated environment
5. Cleanup              → Sandbox destroyed after session ends
```
...
| `settings is not None` | V1 API 需要用户设置 | 通过前端配置 LLM 设置 |
