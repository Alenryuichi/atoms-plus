...
def response_to_actions(
    ...
    if hasattr(assistant_msg, 'tool_calls') and assistant_msg.tool_calls:
        ...
        for i, tool_call in enumerate(assistant_msg.tool_calls):
            ...
            # ================================================
            # Atoms Plus: delegate_to_role (Role-based delegation)
            # ================================================
            elif tool_call.function.name == 'delegate_to_role':
                role = arguments.get('role', 'engineer')
                task = arguments.get('task', '')
                context = arguments.get('context', '')
                expected_output = arguments.get('expected_output', '')

                # Create delegation action with role-specific inputs
                action = AgentDelegateAction(
                    agent='CodeActAgent',  # Use CodeActAgent with role prompt
                    inputs={
                        'task': task,
                        'role': role,
                        'context': context,
                    ...
                    thought=f'Delegating to {role}: {task[:100]}...',
                )
